    ...
    def test_native_dialog(e):
        ...
        try:
            ...
            result = subprocess.run(
                ["osascript", "-e", 'choose folder with prompt "Choose a folder"'],
                capture_output=True,
                text=True,
                timeout=10,
            )
    ...
    page.add(
        ft.Column(
            [
                ...
                ft.ElevatedButton(
                    "Directory Picker with Title", on_click=select_folder_with_title
                ),
                ft.Container(height=10),
                ft.ElevatedButton(
                    "Directory Picker with Initial Dir",
                    on_click=select_folder_with_initial_dir,
                ),
                ft.Container(height=10),
                ft.ElevatedButton(
                    "File Picker (for comparison)", on_click=test_file_picker
                ),
                ft.Container(height=10),
                ft.ElevatedButton(
                    "Native Dialog (osascript)", on_click=test_native_dialog
                ),
                ft.Container(height=20),
                result_text,
            ]

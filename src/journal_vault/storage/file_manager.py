...
class FileManager:
    ...
    def __init__(self, vault_path: str):
        ...
        self.metadata_path = self.vault_path / ".journal_vault"
    ...
    def _get_entry_file_path(self, entry_date: date) -> Path:
        ...
        year_month_dir = self.entries_path / str(entry_date.year) / f"{entry_date.month:02d}"
    ...
    def create_entry(self, entry_date: date, title: str = None, content: str = "") -> JournalEntry:
    ...
    def load_entry(self, entry_date: date) -> Optional[JournalEntry]:
        ...
        try:
            ...
            # Create entry object
            entry = JournalEntry(
                title=frontmatter.get('title', f"Journal Entry - {entry_date.strftime('%B %d, %Y')}"),
                content=content,
                ...
                word_count=frontmatter.get('word_count', len(content.split()) if content else 0),
    ...
    def delete_entry(self, entry_date: date) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("DELETE FROM entries WHERE date = ?", (entry_date.isoformat(),))
                conn.commit()
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor.fetchall():
                    entry_date = date.fromisoformat(row[0])
                    entry = self.load_entry(entry_date)
                    if entry and (query_lower in entry.content.lower() or 
        ...
        try:
            for md_file in self.entries_path.glob("*/*/*.md"):
                ...
                try:
                    entry_date = datetime.strptime(filename, "%Y-%m-%d").date()
                    with sqlite3.connect(self.db_path) as conn:
                        cursor = conn.execute("SELECT 1 FROM entries WHERE date = ?", (entry_date.isoformat(),))

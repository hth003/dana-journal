...
class DownloadProgress:
    ...
    status: str = "initializing"  # "initializing", "downloading", "validating", "complete", "error"
...
class ModelDownloadManager:
    ...
    def check_system_requirements(self) -> Dict[str, Any]:
        ...
        try:
            ...
            requirements["meets_requirements"] = False
            requirements["issues"].append(f"Error checking system requirements: {str(e)}")
        
        return requirements
    
    def download_model_async(self, progress_callback: Optional[Callable[[DownloadProgress], None]] = None) -> None:
    ...
    def _download_model_sync(self, progress_callback: Optional[Callable[[DownloadProgress], None]] = None) -> None:
        ...
        try:
            ...
            if "404" in str(e):
                self.progress.error_message = "Model not found. Please check your internet connection."
            else:
                self.progress.error_message = f"Download failed: {str(e)}"
            self.progress.status = "error"
            update_progress()
        except RepositoryNotFoundError:
            self.progress.error_message = "Model repository not found. The model may have moved."
            self.progress.status = "error"
            update_progress()
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if target_path.exists():
                    ...
                    self.progress.total_bytes = max(self.progress.total_bytes, self.progress.bytes_downloaded)
                    progress_callback()
                
                return target_path
                
            except Exception as e2:
                raise Exception(f"Both download methods failed. Direct: {e}, HF Hub: {e2}")
    ...
    def _download_direct_with_progress(self, progress_callback: Callable) -> Path:
        ...
        try:
            ...
            total_size = int(response.headers.get('content-length', self.MODEL_SIZE_BYTES))
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    ...
                    if chunk:
                        ...
                        if current_time - last_update_time > 0.5:
                            ...
                            if time_elapsed > 0:
                                self.progress.download_speed = bytes_downloaded / time_elapsed
                                
                                # Estimate time remaining
                                remaining_bytes = total_size - bytes_downloaded
                                if self.progress.download_speed > 0:
                                    self.progress.eta_seconds = int(remaining_bytes / self.progress.download_speed)
